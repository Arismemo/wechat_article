...
class Phase2QueueServiceTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_enqueue_is_deduplicated_and_acknowledged(self) -> None:
        redis = FakeRedis()
        service = Phase2QueueService(redis_client=redis)
    ...
    def test_requeue_processing_jobs_moves_items_back(self) -> None:
        redis = FakeRedis()
        redis.processing = ["task-a", "task-b"]
        redis.pending = {"task-a", "task-b"}
        service = Phase2QueueService(redis_client=redis)
        ...
        self.assertEqual(redis.processing, [])
        self.assertEqual(redis.queue, ["task-a", "task-b"])
