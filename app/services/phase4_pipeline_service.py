    ...
    def _build_generation_payload(
        ...
        *,
        ...
        user_prompt = (
            ...
            "## 标题（title 字段填最优者，其余候选写进 markdown_content 开头注释除外，仅供你择优）\n"
            "- 先在心里产出 5 个标题候选，覆盖不同套路：数字式/悬念式/痛点式/人群式/损失厌恶式各 1，再择优填入 title。\n"
            "- 每个候选：≤28 字且核心钩子落在前 15 字内；至少含 1 个具体数字（禁“很多/大量”）；"
            "至少锁定一类人群/场景标签；至少命中“人群/场景+数字/热点+痛点/悬念+方案/反转+情绪词”5 槽位中的 3 个。\n"
            "- 优先损失厌恶框架（别再/千万不要/警惕），而非收益框架（学会…技巧）；悬念必须带具体锚点，禁纯“你绝想不到”；初中生能秒懂。\n"
            "## 开头（前 50 字定生死）\n"
            "- 前 50 字内必须出现【悬念/数据/故事】至少一种，同时做完：抓注意 + 让读者确认“写给我的” + 暗示即将获得的价值。\n"
            "- 开头三选一模板：①未命名的“那一个”（有个习惯把高手和普通人分开）②反直觉结果（我做了 X，结果 Y 翻倍）③条件式威胁（如果你正在这么做…）。把反常识结论前置到开篇。\n"
            "- 每约 300 字埋一个新钩子（悬念/反转/未答问题/小标题），每段结尾给读者继续读下去的理由。\n"
            "## 去 AI 味·词级\n"
            "- 排序词（首先/其次/最后）整篇 ≤1 次，改“起初…后来…现在才明白”。\n"
            "- 禁用书面大词：显而易见/不言而喻/毋庸置疑/鉴于/相应地/众所周知/让我们；禁非必要黑话：矩阵/赋能/生态链路/输出。\n"
            "- 绝对表述降级：所有人都→大多数；必然→往往会；是事实→据说。主动撒入模糊/语气词：其实/说实话/大概/还挺/有点/好像。\n"
            "## 去 AI 味·句式级\n"
            "- 长短句交替：每段混排 8-10 字短句与 20-25 字长句（避免句长趋同）；允许单句成段；全文 ≥1 处疑问句 +1 处感叹句；主动语态 ≥60%。\n"
    ...
    def _build_review_payload(
        ...
        *,
        ...
        user_prompt = (
            ...
            "## 扩展打分维度（沿用上述字段，勿新增字段；命中的具体问题写进 issues，改写建议写进 suggestions）\n"
            "- ai_trace_score（0-100，越高表示越像 AI/机械腔）须综合以下信号：句长方差过小（句长趋同）；"
            "排序词“首先/其次/最后”出现 >1 次；命中书面大词黑名单（显而易见/不言而喻/毋庸置疑/鉴于/相应地/众所周知/让我们）；"
            ...
            "是否有 ≥1 身份金句与 ≥1 个可截图单元；是否显式点名转发受益人群；敏感/私密话题是否已转第三人称；"
            "结尾是否单一 CTA（多 CTA 扣分）、是否第一人称+强动词+福利钩子+紧迫词、是否有推荐语+互动话题、是否戛然而止无收口。缺失项写进 suggestions 给出可落地改法。\n\n"
            f"原文标题：{source.title or '未知'}\n"
            f"原文摘要：{source.summary or '无'}\n"
            f"新角度：{brief.new_angle or '未提供'}\n"
            f"必须覆盖：{self._json_items(brief.must_cover)}\n"
